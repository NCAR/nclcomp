  ...
  size_fo = size_leftmost * nfo;
    ...
    for( i = 0; i < size_leftmost; i++ ) {
        if(xi->ndim > 1 || i == 0) {
            ...
            /*
                coerce_subset_input_double(xi,tmp_xi,index_xi,xi->type,nxi,0,NULL,NULL);
            */
            }
            else {
        ...
        if(yi->ndim > 1 || i == 0) {
            ...
            /*
                coerce_subset_input_double(yi,tmp_yi,index_yi,yi->type,nyi,0,NULL,NULL);
            */
            }
            else {
        ...
        /*
            coerce_subset_input_double(fi,tmp_fi,index_fi,fi->type,nfi,0,NULL,NULL);
        */
        }
        else {
...
int linint2_public(ncomp_array_old xi, ncomp_array_old yi, ncomp_array_old fi,
                   ncomp_array_old xo, ncomp_array_old yo, ncomp_array_old* fo,
                   int icycx, double xmsg, int iopt) {
    return linint2_alloc_work(xi.addr, xi.ndim, xi.shape, xi.type,
...
int linint2(const ncomp_array* xi, const ncomp_array* yi, const ncomp_array* fi,
            ...
            int icycx, double xmsg, int iopt)
{
    return linint2_alloc_work_struct(xi, yi, fi, xo, yo, fo, icycx, xmsg, iopt);
}
...
int linint2_alloc_output(
    ...
    int i;
    size_t* fo_shape = (size_t*)calloc(fi->ndim, sizeof(size_t));

    for (i = 0; i < fi->ndim - 2; i++)
        fo_shape[i] = fi->shape[i];
    fo_shape[fi->ndim-2] = yo->shape[0];
    fo_shape[fi->ndim-1] = xo->shape[0];

    *fo = ncomp_array_alloc_with_data(fi->type, fi->ndim, fo_shape);

    int ier = linint2_alloc_work_struct(xi, yi, fi, xo, yo, *fo,
                                        icycx, xmsg, iopt);
    free(fo_shape);
    return ier;
}
...
*/
int linint2_struct(ncomp_array_old xi, ncomp_array_old yi, ncomp_array_old fi,
                   ncomp_array_old xo, ncomp_array_old yo, ncomp_array_old* fo,
                   int icycx, double xmsg, int iopt) {
    return linint2_alloc_work(xi.addr, xi.ndim, xi.shape, xi.type,
                   yi.addr, yi.ndim, yi.shape, yi.type,
...
int linint2_struct_new(ncomp_array* xi, ncomp_array* yi, ncomp_array* fi,
